...
import type { Agent, AgentCredentialInput, TModelsConfig } from 'librechat-data-provider';
...
const MAX_AGENT_CREDENTIALS = 10;
const credentialSelector = z.string().max(512).optional();

const normalizeUrlOrigin = (value: string): string => new URL(value).origin;
...
const agentCredentialSchema: z.ZodType<AgentCredentialInput> = z
  .object({
    id: z.string().optional(),
    label: z.string().max(128).optional(),
    origin: z.string().url().refine(isHttpUrl, {
      message: 'Credential origin must be an HTTP(S) URL',
    }),
    loginUrl: z
      .string()
      .url()
      .refine(isHttpUrl, {
        message: 'Credential login URL must be an HTTP(S) URL',
      })
      .optional(),
    authType: z.literal('basic_login').optional(),
    username: z.string().max(512).optional(),
    password: z.string().max(4096).optional(),
    usernameSelector: credentialSelector,
    passwordSelector: credentialSelector,
    submitSelector: credentialSelector,
    successSelector: credentialSelector,
    enabled: z.boolean().optional(),
  })
  .superRefine((credential, ctx) => {
    const origin = normalizeUrlOrigin(credential.origin);
    if (!credential.loginUrl) {
      return;
    }
    if (normalizeUrlOrigin(credential.loginUrl) === origin) {
      return;
    }
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['loginUrl'],
      message: 'Credential login URL must use the same origin as the credential origin',
    });
  })
  .transform((credential) => ({
    ...credential,
    authType: credential.authType ?? 'basic_login',
    origin: normalizeUrlOrigin(credential.origin),
  }));

const agentCredentialsSchema = z.array(agentCredentialSchema).max(MAX_AGENT_CREDENTIALS).optional();

/** Base agent schema with all common fields */
export const agentBaseSchema: z.ZodObject<z.ZodRawShape> = z.object({
...
export const agentCreateSchema: z.ZodObject<z.ZodRawShape> = agentBaseSchema.extend({
  provider: z.string(),
...
export const agentUpdateSchema: z.ZodObject<z.ZodRawShape> = agentBaseSchema.extend({
